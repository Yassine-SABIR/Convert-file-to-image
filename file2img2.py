...
def file2img(file_path):
    ...
    for i in range(0,len_file,3):
        ...
        if i == len_file-1:
            img[x][y]=(content[i],0,0)
        elif i == len_file-2:
            img[x][y]=(content[i],content[i+1],0)
        else:
            img[x][y]=(content[i],content[i+1],content[i+2])
...
def img2file(img,file_path): #this function turns an image into a file and store it in file_path
    ...
    for i in range(n):
        for j in range(m):
            file.write(img[i][j][0])
            file.write(img[i][j][1])
            file.write(img[i][j][2])
    file.close()
